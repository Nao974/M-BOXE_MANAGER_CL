...
class Mboxe():
    ...
    def __init__(self,adresse):
        ...
        self.id = adresse     
        self.versionCode_ref={"read":0x2B, "write":0x3B, "type":float(), "size":4, "step":0.01}
        self.versionCode = 0
        ...
        self.limitBW_ref={"read":0x20, "write":0x30, "type":int(), "size":2, "min":350, "max":750, "step":1}
        self.limitBW = self.limitBW_ref["min"]
        self.limitFW_ref={"read":0x21, "write":0x31, "type":int(), "size":2, "min":750, "max":1150, "step":1}
        self.limitFW = self.limitFW_ref["max"]
        self.offset_ref={"read":0x22, "write":0x32, "type":bytes(), "size":1, "min":-127, "max":+127, "step":1}
        self.offset = 0
        self.deadBand_ref={"read":0x23, "write":0x33, "type":int(), "size":2, "min":0, "max":400, "step":1}
        self.deadBand = self.deadBand_ref["min"]

        self.kpPunch_ref={"read":0x24, "write":0x34, "type":float(), "size":4, "min":0, "max":655.35, "step":0.01}
        self.kpPunch = self.kpPunch_ref["min"]
        self.kdDumping_ref={"read":0x25, "write":0x35, "type":float(), "size":4, "min":0, "max":655.35, "step":0.01}
        self.kdDumping = self.kdDumping_ref["min"]
        self.kiStretch_ref={"read":0x26, "write":0x36, "type":float(), "size":4, "min":0, "max":655.35, "step":0.01}
        self.kiStretch = self.kiStretch_ref["min"]

        self.currentMaxSet_ref={"read":0x28, "write":0x38, "type":bytes(), "size":1, "min":1, "max":50, "step":1}
        self.currentMaxSet = 4.0
        self.protectionGoSet_ref={"read":0x29, "write":0x39, "type":bytes(), "size":1, "min":10, "max":255, "step":1}
        self.protectionGoSet = 20
        self.temperatureMaxSet_ref={"read":0x2A, "write":0x3A, "type":bytes(), "size":1, "min":1, "max":255, "step":1}
        self.temperatureMaxSet = 102

        self.positionCurrent_ref={"read":0x40, "write":0x50, "type":int(), "size":2, "min":self.limitBW, "max":self.limitFW, "step":1}
        self.positionCurrent = 750
        self.speedCurrent_ref={"read":0x41, "write":0x51, "type":bytes(), "size":1, "min":0, "max":255, "step":1}
        self.speedCurrent = 0
